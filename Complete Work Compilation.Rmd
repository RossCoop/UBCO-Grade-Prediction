...
```{r loading studentgrades with .1 .2}
load(file = "studentgrades_1.RData")
head(studentgrades)
```

This variation of studentgrades has each student represented as rows, and courses as columns. Course grades are recorded for each attempt a student takes at each course and creates new columns for new grades. For example, MATH 101 has 3 columns: MATH.101.1, MATH.101.2, MATH.101.3. One for each new attempt at MATH 101
...
```{r loading studentgrades using most recent grade}
...
## Determining Error.. may not be needed as I've got it going up above...
### QQ-plots
### Bootstrapping
