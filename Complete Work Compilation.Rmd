...
# loading new data
load(file="rdata-files/new_merged_data.RData")
...
load(file = "rdata-files/studentgrades_1.RData")
...
studentgrades
# save(studentgrades, file="rdata-files/StudentGrades.RData")
```

#### Loading studentgrades_new

```{r loading studentgrades with new student data, eval=TRUE, echo=FALSE}
load("rdata-files/StudentGrades.RData")
...
load("rdata-files/StudentGrades_old.RData")
...
save(course_profs, file = "rdata-files/course_profs.RData")
```
...
save(rel_course_profs, file = "rdata-files/cleaned_profs.RData")
```

```{r loading professor cleaning and final data, eval=TRUE, echo=FALSE}
# course_profs
load("rdata-files/course_profs.RData")
# rel_course_profs
load("rdata-files/cleaned_profs.RData")
```
...
save(studentgrades_prof, file="rdata-files/Prof_StudentGrades.RData")
```

#### Loading studentgrades_prof_new

```{r loading studentgrades_prof using new student data, eval=TRUE, echo=FALSE}
load(file="rdata-files/Prof_StudentGrades.RData")
...
load(file="rdata-files/Prof_StudentGrades_old.RData")
...
```{r bootstrap gbm error bars, eval=TRUE, echo=FALSE, cache=TRUE}
...
save(model_perf, file = "rdata-files/model_perf.RData")
```
...
load("rdata-files/model_perf.RData")
...
# sorting all models into increasing rmse order
all_model_rmse <- all_model_rmse[order(all_model_rmse$RMSE),]
...
Many of the earlier models proved to be ineffective for this problem containing mostly sparse data. There may be a way using neural networks to perform a time series analysis and prediction, but I didn't have enough time to fully test it. Models including MissForest (except one) use the final test grades to impute the missing predictor grades, so predictions done using this form of imputation leak information and shouldn't be used. 

The best model I found was the gradient boosted machines which were able to handle spare data with ease. Most of the main analysis on these methods were done predicting STAT 230 as it had the most student grades to train/test on of all second year courses. For future improvements on GBMs, one could continue training hyperparameters on different courses or have a different setup for the training/testing data. Other than that I have pushed GBMs as much as they go. I wasn't able to find any models which could handle sparse data as well, but I do think a well designed neural network has lots of potential. I never saved my results from NN predictions, but I averaged 9-11 rmse across the different model structures.
