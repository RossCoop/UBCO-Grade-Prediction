...
```{r finding number of times courses were taken, eval=TRUE}
...
```{r extracting gender, citizenship, and residency from new student rows, eval=FALSE}
# Loading packages
library(dplyr)
...
```{r loading professor cleaning and final data, eval=FALSE}
# course_profs
...
```{r Making factored dataset, eval=FALSE}
...
```{r linear regression on stat.230 prerequisites, eval=TRUE}
...
```{r missForest Impute into RF on course first attempts, eval=TRUE, warning=FALSE}
...
```{r missForest Impute into RF on all studentgrades_prof courses, eval=TRUE}
...
```{r missForest impute into GBM on studentgrades_rep 4 courses, eval=TRUE}
...
all_model_rmse <- rbind(all_model_rmse, c(rmse, "MissForest imputation into Random Forest on 4 courses (studentgrades_rep)"))
## 109 mse.. better still
```

```{r missForest impute into GBM on studentgrades 4 courses, eval=TRUE}
# now, does imputing the data before GBM work better?
# preparing all student grades with stat230
...
all_model_rmse <- rbind(all_model_rmse, c(rmse, "MissForest imputation into Random Forest on 4 courses (studentgrades)"))
```

```{r missForest impute into GBM on studentgrades all courses, eval=TRUE}
# now, does imputing the data before GBM work better?
...
rmse <- sqrt(mean((predictions - stat230.imputed[test_indices,]$STAT.230)^2))
all_model_rmse <- rbind(all_model_rmse, c(rmse, "MissForest imputation into Random Forest on all courses (studentgrades)"))
```
...
```{r GBM data prep COSC.221, eval=FALSE}
# Identifying the course of interest
COI <- "COSC.221"
coursegrades <- studentgrades_prof[!is.na(studentgrades_prof[,COI]),-c(1,3,5)]
...
```{r GBM model training, eval=TRUE, warning=FALSE}
set.seed(5934)
n_rows <- nrow(coursegrades)
train_indices <- sample(1:n_rows, 0.8 * n_rows)  # 80% of rows for training
test_indices <- setdiff(1:n_rows, train_indices) # remaining rows for testing
...
```{r GBM on studentgrades_prof_new, eval=TRUE}
# Identifying the course of interest
COI <- "STAT.230"
...
```{r GBM on studentgrades_new, eval=TRUE}
# Identifying the course of interest
COI <- "STAT.230"
...
```{r GBM on studentgrades, eval=TRUE}
# Identifying the course of interest
COI <- "STAT.230"
...
```{r GBM data prep many courses, eval=TRUE}
# STAT 230
COI <- "STAT.230"
coursegrades <- studentgrades_prof[!is.na(studentgrades_prof[,COI]),-c(1,3,5)]
...
```{r bootstrap gbm error bars, eval=TRUE}
