...
for(i in 20:36){
  coursegrades[,i] <- factor(coursegrades[,i]) 
}
...
# removing professor columns which have only one professor
one_prof_courses <- numeric(0)
for(i in 1:ncol(coursegrades)) {
  if(length(unique(coursegrades[,i])) <= 2){   ## 2 being <NA> and the one prof
    one_prof_courses <- c(one_prof_courses,i)
  }
}
# sbusetting out professor columns w/ one prof
coursegrades <- coursegrades[,-one_prof_courses]
