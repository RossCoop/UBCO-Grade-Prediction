...
train_indices <- sample(1:n_rows, 0.85 * n_rows)  # 80% of rows for training
...
gbm_params <- list(
  ...
  n.trees = 1000,               # Number of trees (iterations)
  interaction.depth = 5,       # Maximum depth of trees
  shrinkage = 0.01,            # Learning rate (shrinkage)
  bag.fraction = 0.5,          # Fraction of training data used for each tree
  train.fraction = 1.0,        # Fraction of training data used for training (1.0 for full dataset)
  n.minobsinnode = 15          # Minimum number of observations in terminal nodes
)
...
```{r adding error bars}
## trying to add error bars to a barplot of grade predictions
errors <- abs(predictions - actual)
error_bars <- errors * 1.6  # Approximate 95% confidence interval

plot(actual, predictions, 
     xlab = "Actual Grades", ylab = "Predicted Grades",
     main = "Predicted vs Actual Grades with Error Bars", xlim = c(55,100), ylim = c(40,110),pch = 16)
