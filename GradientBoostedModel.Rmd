...
## GBM Classification
```{r}
load(file = "StudentGradesLetters.RData")
studentgrades
```
...
# Apply the conversion function to all columns except the first 5 columns
studentgrades <- studentgrades %>% mutate(across(-c(1:5), convert_to_binary))
```
...
# Factoring all columns
# for(i in 1:ncol(coursegrades)){
#   coursegrades[,i] <- as.factor(coursegrades[,i])
# }
```
...
gbm_params <- list(
  distribution = "adaboost",  # Specify the distribution for regression
  n.trees = 200,               # Number of trees (iterations)
  interaction.depth = 4,       # Maximum depth of trees
  shrinkage = 0.05,            # Learning rate (shrinkage)
  bag.fraction = 0.5,          # Fraction of training data used for each tree
...
Tried classification on letter grades. Turns out it must be a binary input so I switched to a pass/fail objective. Where "F" grades are 0, and everything else is 1. ntrees is predicting well and works on OOB and CV. However the prediction results range from -0.036 to 3... not sure what to make of it.
