...
gbm_params <- list(
  ...
  interaction.depth = 2,       # Maximum depth of trees
  shrinkage = 0.001,            # Learning rate (shrinkage)
  bag.fraction = 0.5,          # Fraction of training data used for each tree
  train.fraction = 1.0,        # Fraction of training data used for training (1.0 for full dataset)
  n.minobsinnode = 5          # Minimum number of observations in terminal nodes
...
plot(actual, predictions, 
     ...
     main = "Predicted vs Actual Grades with Error Bars", xlim = c(50,100), ylim = c(50,100),pch = 16)
...
gbm_params <- list(
  distribution = list(name = "quantile", alpha = 0.05),  # Specify the distribution for regression
  n.trees = 8000,               # Number of trees (iterations)
  interaction.depth = 2,       # Maximum depth of trees
  shrinkage = 0.001,            # Learning rate (shrinkage)
  bag.fraction = 0.5,          # Fraction of training data used for each tree
...
gbm_params <- list(
  distribution = list(name = "quantile", alpha = 0.95),  # Specify the distribution for regression
  n.trees = 8000,               # Number of trees (iterations)
  interaction.depth = 2,       # Maximum depth of trees
  shrinkage = 0.001,            # Learning rate (shrinkage)
  bag.fraction = 0.5,          # Fraction of training data used for each tree
...
```{r comparing predicted, actual, and student mean grades}
comp <- data.frame(pred=predictions, actual=actual)

avgs <- rowMeans(coursegrades[test_indices,], na.rm = TRUE)
comp <- cbind(comp, avgs)

plot(avgs~pred, data=comp, xlim=c(0,100), ylim=c(0,100))

barplot(t(as.matrix(comp[20:29,])), beside=TRUE)
```
...
xgbm_model <- xgb.train(data=train_data, max.depth=2, eta=0.05, nthread = 2, nrounds=200, lambda = 2, booster = "gbtree", subsample = 0.5, tree_method = "auto", watchlist=watchlist, objective = "reg:squarederror", verbose = 2)
