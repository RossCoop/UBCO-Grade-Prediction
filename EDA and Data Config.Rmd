...
```
```{r}
save(df_bsc, file = "student-data.RData")
```

```{r}
df_clean = df_clean[,-c(15:17)]
```
...
# sorted_courses_taken_bsc <- sorted_courses_taken_bsc[substr(names(sorted_courses_taken_bsc),1,4) %in% c("DATA","MATH",
                                                   # "COSC","STAT")]
sorted_courses_taken_bsc <- sort(courses_taken_bsc[courses_taken_bsc > 25], decreasing = TRUE)
...
save(studentgrades, file = "studentgrades_1.RData")
```
...
```{r}
# load(file = "StudentGrades_old.RData")

save(studentgrades, file = "StudentGrades_old.RData")
```




```{r adding all csvs together}
...
save(studentgrades_prof, file="Prof_StudentGrades_old.RData")
```
...
# Return student with at least 3 grades in studentgrades
...
plot(NA, xlim=c(1, length(unique(data$course_name))), ylim = range(data$grade),
...
for (ids in data[data$grade<= 90 & data$grade >= 49 & data$course==obj_course,]$student) {
...
# number of courses who have been taken by at least x people: 25 people = 105 courses | 250 people = 25 courses
length(sorted_courses_taken_bsc[sorted_courses_taken_bsc >= 25])
...
```{r missForest Impute on all courses USED FOR TESTING ON DIFF COURSES (COSC.221)}
