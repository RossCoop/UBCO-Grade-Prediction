...
# Extract the year from the selected course
course_year <- as.integer(sub("^[^.]+\\.([0-9]+).*", "\\1", COI))

# Find columns with digits not matching the selected course year
cols_to_remove <- column_names[column_digits > course_year]

# Remove irrelevant columns
coursegrades <- coursegrades[, !colnames(coursegrades) %in% cols_to_remove]
...
mse
# around 115.. pretty good for first attempts
```
```{r}
colnames(coursegrades)
colnames(studentgrades)
```
