...
duplicate_rows <- df_bsc[df_bsc(df_bsc[c("STUD_NO_ANONYMOUS", "COURSE_CODE")]) | duplicated(df_bsc[c("STUD_NO_ANONYMOUS", "COURSE_CODE")], fromLast = TRUE), ]
...
for (i in 1:nrow(df_bsc)){
  rep <- 0
  # finding the index of course name .1
  coursecol <- grep(df_bsc[i,]$COURSE_CODE, colnames(studentgrades))[1]
  
  # Moving over from .1 to .2 columns if a grade already exists
  while(!is.na(studentgrades[studentgrades$Student_ID==df_bsc[i,]$STUD_NO_ANONYMOUS,coursecol+rep])){
    rep = rep+1
  }
  
  # Saving student grade into studentgrades df
  studentgrades[studentgrades$Student_ID==df_bsc[i,]$STUD_NO_ANONYMOUS,coursecol+rep] <- df_bsc[i,]$HDR_CRS_PCT_GRD
}
...
```{r}
studentgrades[studentgrades$Student_ID=="61DA78B68FA49F369B99DF05D981B54D",]
