...
coursedata <- df_bsc[df_bsc$STUD_NO_ANONYMOUS %in% studs,]
#coursedata <- coursedata[coursedata$COURSE_CODE == "STAT.230" | coursedata$COURSE_CODE == "MATH.101" | coursedata$COURSE_CODE == "COSC.221" | coursedata$COURSE_CODE == "DATA.101",]
coursedata <- coursedata[order(coursedata$COURSE_CODE),]

data <- data.frame(
  student = coursedata$STUD_NO_ANONYMOUS,
  course = coursedata$COURSE_CODE,
  grade = coursedata$HDR_CRS_PCT_GRD
)
...
# Create training vs test sets
n_rows <- nrow(data)
splitr <- as.integer(n_rows*0.9)
#train_indices <- sample(1:n_rows, 0.8 * n_rows)  # 80% of rows for training
#test_indices <- setdiff(1:n_rows, train_indices) # remaining rows for testing
train_indices <- 1:splitr
test_indices <- (splitr+1):n_rows


# Define the training set
train_set <- data_memory(user_index = data$student[train_indices], item_index = data$course[train_indices], rating = data$grade[train_indices])
...
predicted_grades <- data[test_indices,]
...
mse <- mean((predicted_grades$grade - data[test_indices,]$grade)^2)
...
# Example data: students, courses, and grades (with NAs for missing grades)
students <- c("Student1", "Student2", "Student3", "Student4")
courses <- c("Course1", "Course2", "Course3")
grades <- matrix(c(90, NA, 80, 70, 85, NA, NA, 95, 100, 60, NA, 90), nrow = 4, byrow = TRUE)
rownames(grades) <- students
colnames(grades) <- courses

# Convert the matrix into a data frame for recosystem
data <- data.frame(
  user = rep(1:nrow(grades), times = ncol(grades)),
  item = rep(1:ncol(grades), each = nrow(grades)),
  rating = as.vector(t(grades))
)
data <- na.omit(data)  # Remove rows with NA ratings
...
# Define the training set
train_set <- data_memory(user_index = data$user, item_index = data$item, rating = data$rating)

# Train the model with ALS
r$train(train_set, opts = list(dim = 10, costp_l2 = 0.1, costq_l2 = 0.1, lrate = 0.1, niter = 20))


# Initialize the matrix to store predicted grades
predicted_grades <- matrix(NA, nrow = nrow(grades), ncol = ncol(grades))

# Predict missing values
for (i in 1:nrow(grades)) {
  for (j in 1:ncol(grades)) {
    if (is.na(grades[i, j])) {
      predicted_grades[i, j] <- r$predict(data_memory(user_index = i, item_index = j))
    } else {
      predicted_grades[i, j] <- grades[i, j]
    }
  }
}

# Convert the predicted grades back to the original format
rownames(predicted_grades) <- students
colnames(predicted_grades) <- courses
...
# Calculate MSE for observed (non-NA) grades
observed_indices <- which(!is.na(grades))
mse <- mean((predicted_grades[observed_indices] - grades[observed_indices])^2)
cat("Mean Squared Error (MSE):", mse, "\n")
