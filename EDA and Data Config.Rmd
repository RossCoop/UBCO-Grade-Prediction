...
```{r making prof lookup table}
course_list <- c("CHEM", "COSC", "DATA", "ECON", "MATH", "PHYS", "PSYO", "STAT")  # list of courses to find
course_list.len <- length(course_list)
year_list <- c(2018:2023)
seasons <- c("S", "W")
year_list <- paste0(rep(year_list, each=2), seasons)  # list of years with term

# list of prof file names
file_names <- paste0("UBCO-", year_list, "-", rep(course_list, each=12), ".csv")
...
# list of all 'prof' names with more than one person
prof_names_full_data[which(prof_lens > 1)]

# df of all 'prof' names with more than one person
course_profs[which(prof_lens > 1),]

# courses we care about rn
courses <- c("PSYO.111", "ENGL.112","MATH.101", "PSYO.121", "CHEM.121", "CHEM.123","COSC.111","ECON.101","COSC.121","MATH.221","PHYS.111","STAT.230","MATH.200","COSC.221","COSC.222","COSC.211","DATA.101")

course_profs[prof_lens > 1 & (course_profs$course_name %in% courses),]
rel_course_profs <- course_profs[course_profs$course_name %in% courses,]

#checking things
sort(prof_lens, decreasing =  TRUE)
sum(prof_lens > 1)
unique(prof_names_all)
...
rownames(ordered_profs)

course_profs[10,]$Professor

# trying it on ^ one column to start
# want to separate and find (and save) the prof whose name has should up more times
```
...
# length(strsplit(rel_course_profs[3,"Professor"],";")[[1]])
...
```{r adding profs to studentgrades}
studentgrades[,colnames(studentgrades) %in% courses]
...
for (i in 1:ncol(coursegrades)) {
  if(is.na(coursegrades[,i]) |> sum()  > 900){
    courses <- c(courses,i)
  }
}
...
```{r}
temp <-cbind(studentgrades$STAT.230,studentgrades$COSC.111) 
temp <- temp[temp[,] > 50]
temp <- na.omit(temp)
cor(temp[,1],temp[,2] , use = "complete.obs")

plot(studentgrades$STAT.230, studentgrades$BIOL.205)

cor(studentgrades$STAT.230, studentgrades$BIOL.205, use = "complete.obs")
```


```{r Structured Learning}
library(bnlearn)
suppressWarnings({
...
rmse
mse <- mean((pred_svm - grades.real)^2)
mse
```
...
plot(actual, predictions, xlab = "Actual Grades", ylab = "Predicted Grades", main = "Predicted vs Actual Grades", pch = 16, col = "blue", xlim = c(0,100), ylim = c(0,100))
abline(0, 1, col = "red")
abline(h = 50, col = "pink", lty = 2)
abline(h = 60, col = "pink", lty = 2)
abline(h = 70, col = "pink", lty = 2)# pink dashed line
...
```{r Trying RFE... doesnt work on NA}
library("dplyr")
library("faux")
...
lasso <- train(STAT.230 ~., data=studentgrades, method='lasso',  trControl=cv_5, na.action = na.exclude)
```
...
```{r}
cor(studentgrades$MATH.100,studentgrades$MATH.101, use = "complete.obs")
```
