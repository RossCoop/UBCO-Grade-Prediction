...
length(unique(df_bsc_relevant_courses$STUD_NO_ANONYMOUS))
length(uniqueids)
```
...
na.omit(course_averages)
barplot(course_averages$Average, names.arg = course_averages$Courses)

# finding out which columns are NA...
non_na_averages <- anti_join(course_averages, na.omit(course_averages))

# this isnt good. bug testing time
```
